...
auto readImage(string_view _filename)
{
  ...
  // Read the data into our array, with a specified stride of how many floats the user requested, 
  // i.e. for fpreal3 we ignore the alpha channel
  input->read_image(TypeDesc::FLOAT, data.get(), sizeof(T), AutoStride, AutoStride);
