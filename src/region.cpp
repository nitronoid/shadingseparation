...
fpreal* Region::getAlbedoIntensity(uinteger _id, const uint2 _imageDim, const uinteger _regionScale)
{
...
RegionData generateRegions(const uint2 _imageDim, const uinteger _regionScale, const fpreal* const _albedoIntensities)
{
  RegionData r;
  ...
  // N = A - (R - 1) <=> N = A - R + 1
  r.m_numRegions = uint2{_imageDim.x - _regionScale +1, _imageDim.y - _regionScale +1};
  ...
  r.m_pixelRegions = std::make_unique<std::vector<Region*>[]>(_imageDim.x * _imageDim.y);
