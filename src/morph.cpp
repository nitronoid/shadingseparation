...
{
  return {
    std::max(0, int(_coord) - int(_width)),
    std::min(_cap, _coord + _width)};
}
} //end namespace anonymous

void erode(fpreal* _in, fpreal* o_out, uinteger2 _imageDim, uinteger2 _structuringElement, uinteger _iter)
{
  const uinteger2 kernelHalf = ((_structuringElement - uinteger2(1u)) * _iter) / 2u;
