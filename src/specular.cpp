...
std::vector<std::vector<uinteger>> initMaterialSets(
    const span<fpreal3> _albedo, uinteger2 _imageDim, uinteger _numSets)
{
  ...
  for (uinteger i = 0u; i < _numSets; ++i)
  {
    ...
    for (auto px : inv[i]) mask[px] = 1.f;
    std::cout<<inv[i].size() / float(numPixels)<<'\n';
    std::fill_n(eroded.get(), numPixels, 0.0f);
    erode(mask.get(), eroded.get(), _imageDim, {3,3}, 35 * (inv[i].size() / float(numPixels)));

    auto& matSet = materialSets[i];
    for (uinteger px = 0u; px < numPixels; ++px)
    {
      if (eroded[px] > 0.0f) matSet.push_back(px);
    }
...
std::vector<std::pair<uinteger,uinteger>> closestColIndices(const span<std::vector<uinteger>> _indexSets, const span<fpreal3> _cols, uinteger _idx, uinteger _numCols)
{
  // our albedo color
  ...
  {
    return glm::distance2(col, _cols[lhs.second]) < glm::distance2(col, _cols[rhs.second]);
  };
...
std::vector<uinteger> closestColIndicess(const span<const uinteger> _indexSet, const span<fpreal3> _cols, uinteger _idx, uinteger _numCols)
{
  // our albedo color
  const auto& col = _cols[_idx];
  // a vector of pairs, first: the set index, second: the index of the pixel
...
void removeOutliers(
    span<std::vector<uinteger>> _materialTypes, const span<fpreal3> _albedo)
{
  ...
  for (const auto& type : _materialTypes)
  {
    for (const auto& px : type)
    {
      ...
      auto count = std::count_if(closest.begin(), closest.end(), [&](const auto& e)
          { return tNum == e.first; }
          );
      if (count >= k / 2)
...
std::vector<std::vector<fpreal>> computeProbability(
    const span<std::vector<uinteger>> _materialSets, const span<fpreal3> _albedo)
{
  const uinteger numPixels = _albedo.size();
  const uinteger k = 10u;
  const fpreal ik = 1.0f/k;
  std::vector<std::vector<fpreal>> probabilities(_materialSets.size());
  for (auto& prob : probabilities) prob.resize(numPixels);

  for (uinteger i = 0u; i < numPixels; ++i)
  {
    // compute set distances
    std::vector<fpreal> distances; distances.reserve(_materialSets.size());
  ...
  //  auto min = *std::min_element(prob.begin(), prob.end());
  //  const auto mm = max - min;
  //  for (uinteger i = 0u; i < numPixels; ++i)
  //  {
  //    prob[i] = (prob[i] - min) / mm;
  //  }
  //}
